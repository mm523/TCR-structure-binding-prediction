...
def predict_from_datasets(atchley, distances, atr, combo):
    ...
    for key in my_datasets:
        ...
        imp = pickle.load(open("path_to_folder/trained_models/"+ key + "_" + combo + "_imputer_good-templ_" + today + ".pkl", "rb"))
        scaler = pickle.load((open("path_to_folder/trained_models/" + key + "_" + combo + "_scaler_good-templ_" + today + ".pkl", "rb")))
        with open("path_to_folder/trained_models/rbf_gammas_" + key + "_" + combo + "_good-templ_" + today + ".txt") as f:
            gammas = f.readlines()
        gammas = [float(x) for x in gammas]
        X_train = np.genfromtxt("path_to_folder/trained_models/X_train_" + key + "_" + combo + "_good-templ.csv", delimiter=",")
    ...
    filename = "path_to_folder/trained_models/PDB_good_templ_"+combo + "_" + today + ".sav"
