...
good_templates = list(distances.structure_id) #this is all
...
for key in datasets_of_interest: #create kernels for each dataset separately
    ...
    np.savetxt("path_to_folder/trained_models/X_train_" + key + "_" + combo_to_run +"_all-templ.csv", X_train, delimiter=",")
    Ktr, gammas = gen_fn.kernel_generators_train(X_train, Y_train, kernel_type)
    Kernels_tr[key]=Ktr
    #use pickle to save imputer, scaler and gammas
    with open("path_to_folder/trained_models/rbf_gammas_" + key + "_" + combo_to_run +"_all-templ_" + today + ".txt","w") as f:
        f.writelines([str(x) + "\n" for x in gammas])
    with open("path_to_folder/trained_models/" + key + "_" + combo_to_run + "_imputer_all-templ_" + today + ".pkl", "wb") as f1:
        pickle.dump(imp, f1)
    with open("path_to_folder/trained_models/" + key + "_" + combo_to_run + "_scaler_all-templ_" + today + ".pkl", "wb") as f2:
...
with open("path_to_folder/trained_models/PDB_all_templ_" + combo_to_run + "_" + today + ".sav", "wb") as f3:
